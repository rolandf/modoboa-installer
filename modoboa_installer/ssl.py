...
class LetsEncryptCertificateAlternative(CertificateBackend):
    """Use an already existing certificate generated by letsencrypt."""

    def __init__(self, *args, **kwargs):
        """Update config."""
        super(LetsEncryptCertificateAlternative, self).__init__(*args, **kwargs)
        self.hostname = self.config.get("general", "hostname")
        self.domain = self.config.get("general", "domain")
        self.config.set("general", "tls_cert_file", (
            "/etc/letsencrypt/live/{}/fullchain.pem".format(self.domain)))
        self.config.set("general", "tls_key_file", (
            "/etc/letsencrypt/live/{}/privkey.pem".format(self.domain)))

    def generate_cert(self):
        """Create a certificate."""
        utils.printcolor(
            "Using a pre-existing certificate made using letsencrypt", utils.YELLOW)
        with open("/etc/cron.d/letsencrypt", "w") as fp:
            fp.write("0 */12 * * * root /usr/local/bin/certbot renew "
                     "--quiet --no-self-upgrade --force-renewal\n")
        #cfg_file = "/etc/letsencrypt/renewal/{}.conf".format(self.hostname)
        #pattern = "s/authenticator = standalone/authenticator = nginx/"
        #utils.exec_cmd("perl -pi -e '{}' {}".format(pattern, cfg_file))
...
def get_backend(config):
    ...
    elif config.get("certificate", "type") == "self-signed":
        return SelfSignedCertificate(config)
    elif config.get("certificate", "type") == "letsencrypt-alternative":
        return LetsEncryptAlternativeCertificate(config)
